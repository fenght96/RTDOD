...
def build_roi_heads(cfg, input_shape):
    ...
    if name == "IODROIHeads":
        ...
        return ROI_HEADS_REGISTRY.get(name)(cfg, input_shape, cls_feats=cls_feats)
...
class IODROIHeads(ROIHeads):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.train_on_pred_boxes = train_on_pred_boxes
        self.cls_fcs = nn.Linear(self.cls_feats.shape[1], 1024)
        self.cls_fcs1 = nn.Linear(1024 * self.cls_feats.shape[0], self.box_predictor.cls_score.in_features)
    ...
    def from_config(cls, cfg, input_shape):
        ...
        # ret['n_cls'] = cfg.MODEL.ROI_HEADS.CUR_NUM_CLASSES
        if inspect.ismethod(cls._init_box_head):
            ret.update(cls._init_box_head(cfg, input_shape))
    ...
    def forward(
        ...
        See :class:`ROIHeads.forward`.
        """
        # features [tensor for cur task, tensor for early task]
        # proposals [proposal for cur task, proposal for early task]
        # targets for all task / [ target for cut task, target for 0,1,2... task] 
        # cls_feats for N * d N for n_cls
        # test flag for test or eval 
        del images
        if self.training:
            assert not torch.jit.is_scripting()
            assert targets is not None, "'targets' argument is required during training"
            proposals = self.label_and_sample_proposals(proposals, targets)
        del targets
        ...
        if self.training:
            ...
            pre_predictions = []
            if pre_mask is None:
                # for cls_feat in self.cls_feats:
                loss, div_loss, gate, _ = self._forward_box(features, proposals, self.cls_feats)
                for k,v in loss.items():
                    if k not in losses.keys():
                        losses[k] = v
                # losses['div_loss'] = div_loss
            else:
                loss, _, _, pre_prediction  = self._forward_box(features, proposals, None, pre_mask)
                if len(pre_prediction) > 0:
    ...
    def dis_forward(
        ...
        for mask in pre_masks:
            ...
            pre_predictions.append(predictions)
    ...
    def _forward_box(self, features: Dict[str, torch.Tensor], proposals: List[Instances], cls_feat=None, pre_mask=None):
        """
        Forward logic of the box prediction branch. If `self.train_on_pred_boxes is True`,
            the function puts predicted boxes in the `proposal_boxes` field of `proposals` argument.
        ...
        if self.training:
            ...
            if cls_feat is not None:
                ...
                # div_loss += sum(gate) / gate.shape[0]
                losses = self.box_predictor.losses(predictions, proposals)
