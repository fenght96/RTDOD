...
__all__ = ["build_resnet_fpn_mask_backbone", "build_retinanet_resnet_fpn_mask_backbone", "FPN_Mask"]


class FPN_Mask(Backbone):
    """
    ...
    def forward(self, x, pre_masks=None):
        ...
        if not self.training:
            bottom_up_features = self.bottom_up(x, pre_masks)
            bottom_up_gate_num = bottom_up_features[1]
            bottom_up_features = bottom_up_features[0] # list[dict(k,v)]
            ...
            div_loss = bottom_up_features[1]
            task_masks = bottom_up_features[2]            
            bottom_up_features = bottom_up_features[0]
            ...
            # results = self.change_shape(results)
            return results, div_loss, task_masks
...
def build_resnet_fpn_mask_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
    ...
    """
    bottom_up = build_resnet_mask_backbone(cfg, input_shape)
...
@BACKBONE_REGISTRY.register()
def build_retinanet_resnet_fpn_mask_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
